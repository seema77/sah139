...
pattern_word_tags_list = []  #[pattern, tag]
ignore_words = ['?', '!',',','.', "'s", "'m"]

train_data_file = open('intents.json').read()
intents = json.loads(train_data_file)

def get_stem_words(words, ignore_words): # findins stem words
    stem_words = []
    for word in words:
...
#converting word and patterns into stem words
...
def preprocess_train_data():
    ...
    train_x = bag_of_words_encoding(stem_words, word_tags_list) #[010000001100000011]
